...
risk: safe
source: adapted
tags: [prompt-engineering, optimization, frameworks, ai-enhancement, templates, quality]
triggers: [optimize, improve, enhance, refine, transform, prompt, framework, engineer, design]
...
This is a **universal skill** that works in any terminal context, not limited to specific project structures.
...
**Objective:** Create the final, polished prompt using selected framework(s).

**Actions:**
1. Apply selected framework(s) to the user's intent
2. Incorporate all identified context, constraints, and requirements
3. Ensure prompt is self-contained and clear
4. Apply language adaptation (match user's language)
...
**User Input:**
```
create prompt for: I need to build a product recommendation system considering purchase history, user preferences, and market trends
```
...
**User Input:**
```
optimize prompt: my Python code isn't working, needs debugging
```
...
This skill is **platform-agnostic** and works in any terminal context. It does not depend on:
