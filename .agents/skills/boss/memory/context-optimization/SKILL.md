...
description: "Extends effective context capacity through strategic compression, masking, caching, and partitioning. Use when context limits constrain task complexity or when reducing token costs."
category: memory
risk: safe
source: adapted
tags: [context, optimization, compression, masking, caching, partitioning, tokens]
triggers: [context-constrained, cost-reduction, latency, long-sessions, context-waste]
---
...
if context_tokens / context_limit > 0.8:
    summary = compact_conversation(conversation)
...
BUDGET = {
    ...
    "docs": 0.20,      # 20% for retrieved documents
    "history": 0.40,   # 40% for conversation history
    "buffer": 0.15     # 15% reserved
}
```
