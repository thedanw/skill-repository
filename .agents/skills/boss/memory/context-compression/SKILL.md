...
description: "Production-ready compression strategies for million-token agent sessions. Covers anchored iterative summarization, opaque compression, and regenerative summaries. Use when agent sessions exceed context window limits or debugging agent forgetfulness."
category: memory
risk: safe
source: adapted
tags: [compression, summarization, tokens, session-management, context, memory]
triggers: [context-exceeded, tokens-per-task, artifact-trail, session-long, agent-forgetfulness]
---
...
Instead, use probe-based evaluation: ask the agent questions about the compressed content and measure whether it can answer correctly.
...
3. **Execution Phase**: Work from the compressed specification. Reference back to full code only when needed. This is the phase where most agent work happens, and the compressed context keeps the agent focused on the implementation plan rather than drowning in code.
